...
options:
    ...
    apply_spec:
        description:
            - Path to the specification file to apply during bootstrap.
        required: false
    single_host_defaults:
        description:
            - Deploy single-host defaults.
        required: false
author:
...
- name: bootstrap a cluster via cephadm with spec file
  cephadm_bootstrap:
    mon_ip: 192.168.42.1
    apply_spec: /path/to/spec/file

- name: bootstrap a cluster via cephadm with single host defaults
  cephadm_bootstrap:
    mon_ip: 192.168.42.1
...
def run_module() -> None:
    ...
    # Extends with bootstrap parameters
